...
{
	class ChainPayWork : AbstractHandler, ISendMessage, ITextMessage, IUserDataBase, IMessage
	{
		Message _message = null;
		User user = null;
		Database db = null;
		String TextMessage = null;
		String TextMessageSend = null;
		Post post = null;

		public override System.Object Handle(System.Int32 request, TelegramBotClient botClient, System.Object message)
		{
			if (request == (Int32)Chain.PayWork)
			{
				IMessage iMessage = new ChainPayWork();
				IUserDataBase userDataBase = new ChainPayWork();
				ITextMessage textMessage = new ChainPayWork();
				...
				TextMessage = textMessage.SetTextMessage(_message, "/ChainPayWork", user.Language);
		...
		{
			await Task.Run(() => botClient.SendText(_message.From.Id, post.PostText));
